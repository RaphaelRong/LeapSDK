...
//          [[frame fingers] count], [[frame tools] count], [[frame gestures:nil] count]);
    ...
    if ([[frame hands] count] != 0) {
        ...
        if ([fingers count] != 0) {
            ...
            LeapFinger *finger = [fingers objectAtIndex:0];
            LeapVector *vector = [finger tipPosition];
            [self.delegate getFingerPosition:CGPointMake(vector.x, vector.y)];
...
//                    NSLog(@"%f", sweptAngle);
                    ...
                    if ([[[circleGesture pointable] direction] angleTo:[circleGesture normal]] <= LEAP_PI/4) {
                        ...
                        [self.delegate onLeapMotionCycleClockwise:sweptAngle];
                    } else {
                        [self.delegate onLeapMotionCycleCounterClockwise:sweptAngle];
                    }
                }
...
//                      swipeGesture.position, swipeGesture.direction, swipeGesture.speed);
                ...
                [self.delegate onLeapMotionSwip:swipeGesture.direction];
